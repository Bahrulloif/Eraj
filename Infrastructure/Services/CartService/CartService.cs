...
public class CartService : ICartService
{
    ...
    public async Task<Response<GetCartDTO>> AddCart(AddCartDTO cart)
    {
        var find = _context.Carts.FirstOrDefaultAsync(c => c.Id == cart.Id);
        if (find == null)
        {
            var mapped = _mapper.Map<Cart>(cart);
            await _context.Carts.AddAsync(mapped);
            await _context.SaveChangesAsync();
            return new Response<GetCartDTO>(System.Net.HttpStatusCode.OK, "Cart added successfully");
        }
        return new Response<GetCartDTO>(System.Net.HttpStatusCode.BadRequest, "Cart already exist");
    }

    public async Task<Response<GetCartDTO>> UpdateCart(AddCartDTO cart)
    {
        ...
        if (find != null)
        {
            ...
            await _context.Carts.AddAsync(mapped);
            await _context.SaveChangesAsync();
            return new Response<GetCartDTO>(System.Net.HttpStatusCode.OK, "Cart updated successfully");
        }
        return new Response<GetCartDTO>(System.Net.HttpStatusCode.NotFound, "Cart Not Found");
    }
